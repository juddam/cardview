...
function CardView (el, options) {
	...
	this.options = {
		direction: 'v',
		effect: 'slide', // cardview variables: rotate | zoom | slide
		// effect: 'scale', // thumbflip variable
		startPage: 0,

		deg: 10, // cardview variable
		// scalenum: .8, // thumbflip variable
		duration: .28,
		perspective: '300px',
		resizePolling: 100,

		dataset: [], //maybe don't need anymore
...
CardView.prototype = {
	...
	_effectZoomInit: function () {
		...
		if ( this.direction < 0 ) {
			cardToStay[utils.style.transform] = 'translate(0,0) scale(0.8)' + this.translateZ;
			cardToStay.opacity = '0';
		} else {
			cardToMove[utils.style.transform] = 'translate(0,0) scale(1)' + this.translateZ;
	...
	_effectZoomEnd: function () {
		...
		// swiping ???
			...
			cardToStay[utils.style.transform] = 'scale(.8)' + this.translateZ;
		}
	},
	...
	_effectSlideInit: function () {
		if ( this.direction < 0 ) { //scrolling down
			if ( this.options.direction == 'v' ) {
				this.cardToMove = this.prevCard; // this.currCard;
				this.cardToStay = this.currCard; //this.prevCard;
			} else { //horizontal
				this.cardToMove = this.currCard;
				this.cardToStay = this.prevCard;
			}
	...
	_effectSlideEnd: function () {
		...
		if ( this.direction > 0 ) {
			if ( this.options.direction == 'v' ) {
				cardToMove[utils.style.transform] = 'translate(0,' + distance + '%)' + this.translateZ;
			} else {
	...
	_effectSlideMove: function (distance) {
		if ( this.options.direction == 'v' ) {
			if ( this.direction > 0 ) { //scrolling up
				distance = 100 + 100 / this.wrapperSize * distance;
			} else {
				distance = 100 / this.wrapperSize * distance;
			}
